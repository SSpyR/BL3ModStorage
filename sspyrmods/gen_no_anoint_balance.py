...
#Still not working but am on to something
...
for bal in bal_name:
    no_anoint_check=False
    for wep in no_anoints:
        if wep in bal:
            no_anoint_check=True
    
    if no_anoint_check==False:
        balref=bal.split('/')
        mod.reg_hotfix(Mod.PATCH, '',
        bal + '.' + balref[(len(balref)-1)],
        'RuntimeGenericPartList.PartList',
        """
        (
            (
                PartData=(GPart_All_SkillEnd_SplashDamage,/Game/Gear/Weapons/_Shared/_Design/EndGameParts/_Generic/SkillEnd_SplashDamage/GPart_All_SkillEnd_SplashDamage),
                Weight=((BaseValueAttribute=Att_EndGame_GenericPartChance,/Game/Gear/Weapons/_Shared/_Design/EndGameParts/DropWeight/Att_EndGame_GenericPartChance),(BaseValueConstant=0),(BaseValueScale=0))
            )
        )
        """
        )
        ...
        #partset=Balance.from_data(data, bal)
        #partset.hotfix_partset_anoint(mod)
        #mod.newline()'''
